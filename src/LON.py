...
class LON:
    def __init__(self, cfg, cities_graph: CitiesGraph):
        ...
        self.name = cfg.experiment.instance
    ...
    def generate_new_solution(self, modification='2-opt'):
        ...
        new_dst = self.cities_graph.calc_total_distance(new_solution)
        if modification == 'swap_random_two':
            ...
            for i in range(1, len(self.solution) - 1):
                for j in range(i + 3, len(self.solution)):
                    beg = self.solution[:i + 1]
                    mid = self.solution[j - 1:i:-1]
                    end = self.solution[j:]
                    tmp_solution = np.concatenate([beg, mid, end]).astype(int)
                    if self.cities_graph.calc_total_distance(tmp_solution) < new_dst:
    ...
    def first_improvement_2opt(self, solution):
        ...
        while is_improved:
            ...
            for i in range(0, len(solution2) - 1):
                ...
                if is_improved:
                    break
                for j in range(i + 2, len(solution2)):
                    beg = solution1[:i + 1]
                    mid = solution1[j:i:-1]
                    end = solution1[j + 1:]
    ...
    def two_opt(self):
        ...
        for i in range(0, len(self.solution) - 1):
            for j in range(i + 2, len(self.solution)):
                beg = self.solution[:i + 1]
                mid = self.solution[j:i:-1]
                end = self.solution[j + 1:]
                tmp_solution = np.concatenate([beg, mid, end]).astype(int)
                if self.cities_graph.calc_total_distance(tmp_solution) < new_dst:
                    new_solution = tmp_solution.copy()
                    new_dst = self.cities_graph.calc_total_distance(new_solution)
    ...
    def sample_edges(self):
        ...
        for node in self.nodes:
            logging.info(f'node: {self.nodes.index(node)}')
    ...
    def calc_con_rel(self):
        ...
        return (sum_con / sum_non_con).tolist()
    ...
    def save_metrics_to_file(self):
        with open(f"{self.name}-metrics.json", "w") as fp:
            json.dump(self.metrics, fp)

        np.save(f"{self.name}-nodes", np.array(self.nodes))
        np.save(f"{self.name}-edges", self.edges)
